...
const RecordingControls: React.FC<{
  ...
  <div className='w-full p-3 border-2 border-dashed border-green-800 bg-black/30 space-y-3'>
    ...
    <div className='flex items-center gap-2'>
      {isRecording ? (
        <button
          ...
          className='arcade-button-small bg-red-600 border-red-500 w-1/2 animate-pulse'
        >
          STOP
        </button>
      ) : (
        <button onClick={onStartRecording} className='arcade-button-small w-1/2'>
          {recordedAudioUrl ? 'AUDIO OK!' : 'REC VOICEOVER'}
        </button>
      )}
    ...
    {isTranscribing && (
      <p className='pixel-font-small text-yellow-400 animate-pulse text-center mt-2'>
        TRANSCRIBING AUDIO...
      </p>
    )}
...
const VideoPlayer: React.FC<{
  ...
  >
    <video
      ...
      aria-label='Generated recipe video'
    >
      <track kind='captions' src='data:text/vtt,WEBVTT' label='No captions' default />
    </video>
    ...
    <div className='w-full p-3 border-2 border-dashed border-green-800 bg-black/30 space-y-3'>
      ...
      <div className='flex items-center gap-2'>
        <label
          ...
          className='arcade-button-small cursor-pointer w-1/2 text-center'
        >
  ...
  <div className='flex flex-col items-center gap-3 mt-4 w-full'>
    <div className='flex flex-wrap gap-2 justify-center'>
      <div className='share-indicator-container'>
        ...
        {showSaveConfirmation && <span className='share-indicator-text'>SAVED TO COOKBOOK!</span>}
      </div>
      {!videoUrl && (
...
const RecipeDetails: React.FC<{
  ...
  <div ref={scrollContainerRef} className='recipe-details-scroll text-left'>
    ...
    <p className='pixel-font-small text-gray-300 mb-4 text-center italic'>
      &quot;{safeDescription}&quot;
    </p>
...
const RecipeResult: React.FC<RecipeResultProps> = ({
  ...
  const [videoGenerationStep, setVideoGenerationStep] = React.useState<'idle' | 'generating'>(
    'idle'
  );
  const [videoLoadingMessage, setVideoLoadingMessage] = React.useState(
    VIDEO_GENERATION_MESSAGES[0]
  );
  ...
  const [uploadedImage, setUploadedImage] = React.useState<{ url: string; base64: string } | null>(
    null
  );
  ...
  const safeDishName = React.useMemo(
    () => escapeText(recipe.dishName),
    [escapeText, recipe.dishName]
  );
  const safeDescription = React.useMemo(
    () => escapeText(recipe.description),
    [escapeText, recipe.description]
  );
  ...
  React.useEffect(() => {
    ...
    const instructionElements =
      instructionsContainerRef.current?.querySelectorAll('.instruction-step');
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        try {
          ...
          const draft = existingDraftJson
            ? (JSON.parse(existingDraftJson) as { imageBase64?: string; audioBase64?: string })
            : {};
      ...
      try {
          ...
          reader.onloadend = () => {
            ...
            if (audioBase64) {
              try {
                const existingDraftJson = localStorage.getItem(DRAFT_KEY);
                const draft = existingDraftJson
                  ? (JSON.parse(existingDraftJson) as {
                      imageBase64?: string;
                      audioBase64?: string;
                    })
                  : {};
        ...
        try {
          ...
          if (existingDraftJson) {
            const draft = JSON.parse(existingDraftJson) as {
              imageBase64?: string;
              audioBase64?: string;
            };
  ...
  const handleGenerateVideo = async () => {
    ...
    try {
      ...
      const generatedUrl = await generateRecipeVideo(
        recipe,
        transcribedText,
      ...
      if (rawMessage.includes('API KEY ERROR') && globalThis.window?.aistudio?.openSelectKey) {
        ...
        const displayMessage =
          err instanceof Error ? err.message : 'An unknown video error occurred.';
      ...
      <div className='modal-content recipe-result-modal'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6 p-1'>
          ...
          <div className='flex flex-col items-center space-y-4'>
            ...
            {videoError && (
              <p className='text-red-500 pixel-font-small animate-shake text-center'>
                {videoError}
              </p>
            )}
            <ShareControls
